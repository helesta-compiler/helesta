...
// inline son to fa
void move_func(IR::NormalFunc *fa, IR::CallInstr *son_call, IR::BB *son_bb) {
  using namespace IR;
  struct InlineState {
      ...
      bb->for_each([&](Instr *instr) {
        ...
        Case(LoadArg, load_arg, instr) {
          UnaryOpInstr *uo_instr = new UnaryOpInstr(
              load_arg->d1, son_call->args.at(load_arg->id), UnaryOp::ID);
          map_reg_f(uo_instr->d1);
          instr1 = uo_instr;
        }
        else Case(ReturnInstr, return_instr, instr) {
          UnaryOpInstr *uo_instr =
              new UnaryOpInstr(son_call->d1, return_instr->s1, UnaryOp::ID);
          map_reg_f(uo_instr->d1);
          bb1->push(uo_instr);
