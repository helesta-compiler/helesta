...
DomTree build_dom_tree(NormalFunc *f) {
  ...
  for (size_t i = idfn.size() - 1;; --i) {
    ...
    if (sw.loop_rt) {
      ...
      loop_tree_for_each(S, w, [&](BB *u) {
        for (BB *u1 : S[u].out) {
          if (!in_loop(S, u1, w))
            sw.loop_exit.insert(u1);
        }
      });
      dbg << "loop: " << w->name << "\n";
      dbg << "```\n";
      dbg << *w;
      for (BB *u : sw.loop_exit)
        dbg << "exit: " << u->name << "\n";
      dbg << "```\n";
    }
    if (!i)
  ...
  for (BB *w : idfn) {
    for (BB *u = S[w].get_loop_rt(); u; u = S[u].loop_fa)
      ++S[w].loop_depth;
  }
  return S;
}
