...
IRValue ASTVisitor::to_IRValue(std::any value) {
    if (!value.has_value()) throw VoidFuncReturnValueUsed();
    if (value.type() == typeid(IRValue)) return std::any_cast<IRValue>(value);
...
CondJumpList ASTVisitor::to_CondJumpList(std::any value) {
    if (!value.has_value()) throw VoidFuncReturnValueUsed();
    if (value.type() == typeid(CondJumpList)) return std::any_cast<CondJumpList>(value);
...
void ASTVisitor::dfs_const_init(SysYParser::ListConstInitValContext *node,
    ...
    for (auto child : node->constInitVal()) {
        if (auto scalar_child =
            ...
            result.push_back(std::any_cast<CompileTimeValue>(scalar_child->constExp()->accept(this)).value);
            ++cnt;
        } else {
            auto list_child =
...
vector<int32_t> ASTVisitor::parse_const_init(
    ...
    if (auto scalar_root =
        ...
        ret.push_back(std::any_cast<CompileTimeValue>(scalar_root->constExp()->accept(this)).value);;
        return ret;
    }
...
std::any ASTVisitor::visitListConstInitVal(
...
std::any ASTVisitor::visitUninitVarDef(
...
std::any ASTVisitor::visitFuncDef(SysYParser::FuncDefContext *ctx) {
    ...
    if (ctx->funcFParams())
        params = std::any_cast<vector<pair<string, Type>>>(ctx->funcFParams()->accept(this));
    FunctionInterface interface;
...
std::any ASTVisitor::visitFuncFParams(
        SysYParser::FuncFParamsContext *ctx) {
    vector<pair<string, Type>> ret;
    for (auto i : ctx->funcFParam()) ret.push_back(std::any_cast<pair<string, Type>>(i->accept(this)));
    return ret;
}
...
std::any ASTVisitor::visitContinueStmt(
...
std::any ASTVisitor::visitPrimaryExp1(
        SysYParser::PrimaryExp1Context *ctx) {
    return ctx->exp()->accept(this);
}

std::any ASTVisitor::visitPrimaryExp2(
        SysYParser::PrimaryExp2Context *ctx) {
    return ctx->lVal()->accept(this);
}

std::any ASTVisitor::visitPrimaryExp3(
...
std::any ASTVisitor::visitUnary2(SysYParser::Unary2Context *ctx) {
    ...
    if (ctx->funcRParams())
        args = std::any_cast<vector<variant<IR::MemObject *, IRValue>>>(ctx->funcRParams());
...
std::any ASTVisitor::visitFuncRParams(
...
std::any ASTVisitor::visitExpAsRParam(
...
std::any ASTVisitor::visitStringAsRParam(
...
std::any ASTVisitor::visitMul2(SysYParser::Mul2Context *ctx) {
    ...
    assert(op == '*' || op == '/' || op == '%');
    if (mode == compile_time) {
        CompileTimeValue lhs = std::any_cast<CompileTimeValue>(ctx->mulExp()->accept(this)),
                         rhs = std::any_cast<CompileTimeValue>(ctx->unaryExp()->accept(this)),
                         res;
        switch (op) {
            case '*':
                res = lhs * rhs;
...
std::any ASTVisitor::visitAdd2(SysYParser::Add2Context *ctx) {
    ...
    assert(op == '+' || op == '-');
    if (mode == compile_time) {
        CompileTimeValue lhs = std::any_cast<CompileTimeValue>(ctx->addExp()->accept(this)),
                         rhs = std::any_cast<CompileTimeValue>(ctx->mulExp()->accept(this)),
                         res;
        switch (op) {
            case '+':
                res = lhs + rhs;
...
std::any ASTVisitor::visitRel2(SysYParser::Rel2Context *ctx) {
    ...
    IR::BinaryOp op{opt};
    if (mode == compile_time) {
        CompileTimeValue lhs = std::any_cast<CompileTimeValue>(ctx->relExp()->accept(this)),
                         rhs = std::any_cast<CompileTimeValue>(ctx->addExp()->accept(this)),
                         res;
...
std::any ASTVisitor::visitEq2(SysYParser::Eq2Context *ctx) {
    ...
    IR::BinaryOp op{opt};
    if (mode == compile_time) {
        CompileTimeValue lhs = std::any_cast<CompileTimeValue>(ctx->eqExp()->accept(this)),
                         rhs = std::any_cast<CompileTimeValue>(ctx->relExp()->accept(this)),
                         res;
...
std::any ASTVisitor::visitLAnd2(SysYParser::LAnd2Context *ctx) {
    if (mode == compile_time) {
        CompileTimeValue lhs = std::any_cast<CompileTimeValue>(ctx->lAndExp()->accept(this)),
                         rhs = std::any_cast<CompileTimeValue>(ctx->eqExp()->accept(this));
        return lhs && rhs;
    } else if (mode == normal) {
...
std::any ASTVisitor::visitLOr2(SysYParser::LOr2Context *ctx) {
    if (mode == compile_time) {
        CompileTimeValue lhs = std::any_cast<CompileTimeValue>(ctx->lOrExp()->accept(this)),
                         rhs = std::any_cast<CompileTimeValue>(ctx->lAndExp()->accept(this));
        return lhs || rhs;
    } else if (mode == normal) {
