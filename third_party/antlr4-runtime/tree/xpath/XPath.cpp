...
std::vector<std::unique_ptr<XPathElement>>
XPath::split(const std::string &path) {
  ...
  while (!done && i < n) {
    ...
    case XPathLexer::ANYWHERE: {
      ...
      std::unique_ptr<XPathElement> pathElement =
          getXPathElement(next, anywhere);
...
std::unique_ptr<XPathElement> XPath::getXPathElement(Token *wordToken,
                                                     bool anywhere) {
  ...
  case XPathLexer::WILDCARD:
    if (anywhere)
      return std::unique_ptr<XPathWildcardAnywhereElement>(
  ...
  case XPathLexer::STRING:
    ...
    if (anywhere)
      return std::unique_ptr<XPathTokenAnywhereElement>(
          new XPathTokenAnywhereElement(word, (int)ttype));
    return std::unique_ptr<XPathTokenElement>(
        new XPathTokenElement(word, (int)ttype));
  ...
  default:
    ...
    if (anywhere)
      return std::unique_ptr<XPathRuleAnywhereElement>(
          new XPathRuleAnywhereElement(word, (int)ruleIndex));
    return std::unique_ptr<XPathRuleElement>(
        new XPathRuleElement(word, (int)ruleIndex));
  }
}
...
XPath::findAll(ParseTree *tree, std::string const &xpath, Parser *parser) {
