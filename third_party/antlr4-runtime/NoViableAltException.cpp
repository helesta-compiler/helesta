...
// Create a normal shared pointer if the configurations are to be deleted. If
// not, then the shared pointer is created with a deleter that does nothing.
Ref<atn::ATNConfigSet> buildConfigsRef(atn::ATNConfigSet *configs,
                                       bool deleteConfigs) {
  if (deleteConfigs) {
...
NoViableAltException::NoViableAltException(
    Parser *recognizer, TokenStream *input, Token *startToken,
    ...
    : RecognitionException("No viable alternative", recognizer, input, ctx,
                           offendingToken),
      _deadEndConfigs(buildConfigsRef(deadEndConfigs, deleteConfigs)),
