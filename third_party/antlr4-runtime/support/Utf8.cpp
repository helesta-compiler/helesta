...
// The below implementation is based off of
// https://github.com/google/cel-cpp/internal/utf8.cc, which is itself based off
// of
// https://go.googlesource.com/go/+/refs/heads/master/src/unicode/utf8/utf8.go.
// If for some reason you feel the need to copy this implementation, please
// retain a comment referencing the two source files and giving credit, as well
// as maintaining any and all obligations required by the BSD 3-clause license
...
std::optional<std::u32string> Utf8::strictDecode(std::string_view input) {
  ...
  size_t codeUnits;
  output.reserve(
      input.size()); // Worst case is each byte is a single Unicode code point.
  for (size_t index = 0; index < input.size(); index += codeUnits) {
    ...
    if (codePoint == Unicode::REPLACEMENT_CHARACTER && codeUnits == 1) {
      // Condition is only met when an illegal byte sequence is encountered. See
      // Utf8::decode.
      return std::nullopt;
    }
...
std::string &Utf8::encode(std::string *buffer, char32_t codePoint) {
  ...
  if (codePoint <= 0x7f) {
    ...
    buffer->push_back(
        static_cast<char>(TX | (static_cast<uint8_t>(codePoint >> 6) & MASKX)));
    buffer->push_back(
        static_cast<char>(TX | (static_cast<uint8_t>(codePoint) & MASKX)));
  } else {
    buffer->push_back(
...
std::optional<std::string> Utf8::strictEncode(std::u32string_view input) {
  ...
  output.reserve(
      input.size() *
      4); // Worst case is each Unicode code point encodes to 4 bytes.
