...
ATNConfigSet::ATNConfigSet(const ATNConfigSet &other)
    : fullCtx(other.fullCtx),
      _configLookup(other._configLookup.bucket_count(), ATNConfigHasher{this},
                    ATNConfigComparer{this}) {
  addAll(other);
...
ATNConfigSet::ATNConfigSet(bool fullCtx)
    : fullCtx(fullCtx),
      _configLookup(0, ATNConfigHasher{this}, ATNConfigComparer{this}) {}
...
bool ATNConfigSet::add(const Ref<ATNConfig> &config,
  ...
  Ref<const PredictionContext> merged = PredictionContext::merge(
  ...
  (*existing)->context =
      std::move(merged); // replace context; no need to alt mapping
...
const Ref<ATNConfig> &ATNConfigSet::get(size_t i) const { return configs[i]; }
...
size_t ATNConfigSet::hashCode() const {
  ...
  if (!isReadonly() || cachedHashCode == 0) {
    ...
    for (const auto &i : configs) {
      cachedHashCode = 31 * cachedHashCode +
...
size_t ATNConfigSet::size() const { return configs.size(); }

bool ATNConfigSet::isEmpty() const { return configs.empty(); }
...
bool ATNConfigSet::isReadonly() const { return _readonly; }

void ATNConfigSet::setReadonly(bool readonly) {
  _readonly = readonly;
  LookupContainer(0, ATNConfigHasher{this}, ATNConfigComparer{this})
      .swap(_configLookup);
}
...
bool ATNConfigSet::equals(const ATNConfig &lhs, const ATNConfig &rhs) const {
  return lhs.state->stateNumber == rhs.state->stateNumber &&
         lhs.alt == rhs.alt && *lhs.semanticContext == *rhs.semanticContext;
}
