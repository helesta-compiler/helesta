...
struct SemanticContextComparer final {
  bool operator()(const SemanticContext *lhs,
                  const SemanticContext *rhs) const {
    return *lhs == *rhs;
  }
};

template <typename Comparer>
void insertSemanticContext(
    const Ref<const SemanticContext> &semanticContext,
    std::unordered_set<const SemanticContext *, SemanticContextHasher,
                       SemanticContextComparer> &operandSet,
  ...
  if (semanticContext != nullptr) {
    if (semanticContext->getContextType() == SemanticContextType::PRECEDENCE) {
      if (precedencePredicate == nullptr ||
          comparer(downCast<const SemanticContext::PrecedencePredicate *>(
                       semanticContext.get())
                       ->precedence,
                   precedencePredicate->precedence)) {
        precedencePredicate = std::static_pointer_cast<
            const SemanticContext::PrecedencePredicate>(semanticContext);
      }
    } else {
    ...
    if (semanticContext->getContextType() == SemanticContextType::PRECEDENCE) {
      if (precedencePredicate == nullptr ||
          comparer(downCast<const SemanticContext::PrecedencePredicate *>(
                   ...
                   precedencePredicate->precedence)) {
        precedencePredicate = std::static_pointer_cast<
            const SemanticContext::PrecedencePredicate>(
            std::move(semanticContext));
      }
    } else {
      auto [existing, inserted] = operandSet.insert(semanticContext.get());
      if (inserted) {
...
size_t predictOperandCapacity(const Ref<const SemanticContext> &a,
                              const Ref<const SemanticContext> &b) {
...
SemanticContext::Predicate::Predicate(size_t ruleIndex, size_t predIndex,
                                      bool isCtxDependent)
    : SemanticContext(SemanticContextType::PREDICATE), ruleIndex(ruleIndex),
      predIndex(predIndex), isCtxDependent(isCtxDependent) {}
...
size_t SemanticContext::Predicate::hashCode() const {
  ...
  hashCode =
      misc::MurmurHash::update(hashCode, static_cast<size_t>(getContextType()));
...
SemanticContext::PrecedencePredicate::PrecedencePredicate(int precedence)
    : SemanticContext(SemanticContextType::PRECEDENCE), precedence(precedence) {
}

bool SemanticContext::PrecedencePredicate::eval(
    Recognizer *parser, RuleContext *parserCallStack) const {
  return parser->precpred(parserCallStack, precedence);
}

Ref<const SemanticContext> SemanticContext::PrecedencePredicate::evalPrecedence(
    Recognizer *parser, RuleContext *parserCallStack) const {
...
size_t SemanticContext::PrecedencePredicate::hashCode() const {
  ...
  hashCode =
      misc::MurmurHash::update(hashCode, static_cast<size_t>(getContextType()));
  hashCode =
      misc::MurmurHash::update(hashCode, static_cast<size_t>(precedence));
  return misc::MurmurHash::finish(hashCode, 2);
}

bool SemanticContext::PrecedencePredicate::equals(
  ...
  const PrecedencePredicate &predicate =
      downCast<const PrecedencePredicate &>(other);
...
SemanticContext::AND::AND(Ref<const SemanticContext> a,
                          Ref<const SemanticContext> b)
    : Operator(SemanticContextType::AND) {
  std::unordered_set<const SemanticContext *, SemanticContextHasher,
                     SemanticContextComparer>
      operands;
  Ref<const SemanticContext::PrecedencePredicate> precedencePredicate;
  ...
  if (a->getContextType() == SemanticContextType::AND) {
    for (const auto &operand : downCast<const AND *>(a.get())->getOperands()) {
      insertSemanticContext(operand, operands, _opnds, precedencePredicate,
                            std::less<int>{});
    }
  } else {
    insertSemanticContext(std::move(a), operands, _opnds, precedencePredicate,
                          std::less<int>{});
  }
  ...
  if (b->getContextType() == SemanticContextType::AND) {
    ...
    insertSemanticContext(std::move(b), operands, _opnds, precedencePredicate,
                          std::less<int>{});
  }
...
const std::vector<Ref<const SemanticContext>> &
SemanticContext::AND::getOperands() const {
  return _opnds;
}
...
SemanticContext::OR::OR(Ref<const SemanticContext> a,
                        Ref<const SemanticContext> b)
    : Operator(SemanticContextType::OR) {
  std::unordered_set<const SemanticContext *, SemanticContextHasher,
                     SemanticContextComparer>
      operands;
  Ref<const SemanticContext::PrecedencePredicate> precedencePredicate;
...
const std::vector<Ref<const SemanticContext>> &
SemanticContext::OR::getOperands() const {
  return _opnds;
}
...
const Ref<const SemanticContext> SemanticContext::NONE =
...
Ref<const SemanticContext> SemanticContext::And(Ref<const SemanticContext> a,
                                                Ref<const SemanticContext> b) {
  if (!a || a == NONE) {
    return b;
  }
...
Ref<const SemanticContext> SemanticContext::Or(Ref<const SemanticContext> a,
                                               Ref<const SemanticContext> b) {
  if (!a) {
    return b;
  }
