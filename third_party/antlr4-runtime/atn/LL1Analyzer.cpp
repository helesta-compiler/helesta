...
public:
  LL1AnalyzerImpl(const ATN &atn, misc::IntervalSet &look, bool seeThruPreds,
                  bool addEOF)
      : _atn(atn), _look(look), _seeThruPreds(seeThruPreds), _addEOF(addEOF) {}
  ...
  /// null}. </param>
  void LOOK(ATNState *s, ATNState *stopState,
            Ref<const PredictionContext> const &ctx) {
    ...
    for (size_t i = 0; i < n; i++) {
      ...
      if (tType == TransitionType::RULE) {
        if (_calledRuleStack[(static_cast<const RuleTransition *>(t))
                                 ->target->ruleIndex]) {
          continue;
        }
        ...
        Ref<const PredictionContext> newContext =
            SingletonPredictionContext::create(
                ctx, (static_cast<const RuleTransition *>(t))
                         ->followState->stateNumber);

        _calledRuleStack.set(
            (static_cast<const RuleTransition *>(t))->target->ruleIndex);
        LOOK(t->target, stopState, newContext);
        _calledRuleStack[(static_cast<const RuleTransition *>(t))
                             ->target->ruleIndex] = false;

      } else if (tType == TransitionType::PREDICATE ||
                 tType == TransitionType::PRECEDENCE) {
        if (_seeThruPreds) {
...
std::vector<misc::IntervalSet>
LL1Analyzer::getDecisionLookahead(ATNState *s) const {
...
misc::IntervalSet LL1Analyzer::LOOK(ATNState *s, ATNState *stopState,
  ...
  Ref<const PredictionContext> lookContext =
