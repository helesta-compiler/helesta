...
void checkCondition(bool condition) { checkCondition(condition, ""); }
    ...
    if (atn.ruleToStartState[state->ruleIndex]->isLeftRecursiveRule) {
      ...
      if (LoopEndState::is(maybeLoopEndState)) {
        if (maybeLoopEndState->epsilonOnlyTransitions &&
            RuleStopState::is(maybeLoopEndState->transitions[0]->target)) {
...
Ref<const LexerAction> lexerActionFactory(LexerActionType type, int data1,
                                          int data2) {
  switch (type) {
...
ConstTransitionPtr edgeFactory(const ATN &atn, TransitionType type, size_t trg,
  ...
  case TransitionType::RULE:
    return std::make_unique<RuleTransition>(
        downCast<RuleStartState *>(atn.states[arg1]), arg2, (int)arg3, target);
...
ATNState *stateFactory(ATNStateType type, size_t ruleIndex) {
  ...
  case ATNStateType::LOOP_END:
    ...
    break;
  default:
    std::string message = "The specified state type " +
                          std::to_string(static_cast<size_t>(type)) +
                          " is not valid.";
...
void deserializeSets(SerializedATNView data, int &p,
...
ATNDeserializer::ATNDeserializer(
    ...
    : _deserializationOptions(std::move(deserializationOptions)) {}

std::unique_ptr<ATN>
ATNDeserializer::deserialize(SerializedATNView data) const {
  int p = 0;
  int version = data[p++];
  ...
  //
  {
    ...
    loopBackStateNumbers.reserve(
        nstates); // Reserve worst case size, its short lived.
    endStateNumbers.reserve(
        nstates); // Reserve worst case size, its short lived.
    for (size_t i = 0; i < nstates; i++) {
      ...
      if (stype == ATNStateType::LOOP_END) { // special case
        ...
        loopBackStateNumbers.push_back(
            {downCast<LoopEndState *>(s), loopBackStateNumber});
      } else if (BlockStartState::is(s)) {
        int endStateNumber = data[p++];
        endStateNumbers.push_back(
            {downCast<BlockStartState *>(s), endStateNumber});
      }
      atn->addState(s);
    }
    ...
    for (auto &pair : endStateNumbers) {
      pair.first->endState =
          downCast<BlockEndState *>(atn->states[pair.second]);
    }
  }
  ...
  for (size_t i = 0; i < numPrecedenceStates; i++) {
    size_t stateNumber = data[p++];
    downCast<RuleStartState *>(atn->states[stateNumber])->isLeftRecursiveRule =
        true;
  }
  ...
  for (size_t i = 0; i < nmodes; i++) {
    ...
    atn->modeToStartState.push_back(
        downCast<TokensStartState *>(atn->states[s]));
  }
  ...
  //
  {
    ...
    for (int i = 0; i < nedges; i++) {
      ...
      ConstTransitionPtr trans =
          edgeFactory(*atn, ttype, trg, arg1, arg2, arg3, sets);
  ...
  for (ATNState *state : atn->states) {
    for (size_t i = 0; i < state->transitions.size(); i++) {
      ...
      const RuleTransition *ruleTransition =
          downCast<const RuleTransition *>(t);
    ...
    if (PlusLoopbackState::is(state)) {
      ...
      for (size_t i = 0; i < loopbackState->transitions.size(); i++) {
        ...
        if (PlusBlockStartState::is(target)) {
          (downCast<PlusBlockStartState *>(target))->loopBackState =
              loopbackState;
        }
      }
    ...
    for (std::vector<RuleStartState *>::size_type i = 0;
      ...
      while (atn->ruleToStartState[i]->transitions.size() > 0) {
        ConstTransitionPtr transition =
            atn->ruleToStartState[i]->removeTransition(
      ...
      atn->ruleToStartState[i]->addTransition(
          std::make_unique<EpsilonTransition>(bypassStart));
      bypassStop->addTransition(std::make_unique<EpsilonTransition>(endState));

      ATNState *matchState =
          new BasicState(); /* mem check: freed in ATN d-tor */
      atn->addState(matchState);
      matchState->addTransition(std::make_unique<AtomTransition>(
          bypassStop, atn->ruleToTokenType[i]));
      bypassStart->addTransition(
          std::make_unique<EpsilonTransition>(matchState));
    }
...
void ATNDeserializer::verifyATN(const ATN &atn) const {
  ...
  for (ATNState *state : atn.states) {
    ...
    if (PlusBlockStartState::is(state)) {
      checkCondition((downCast<PlusBlockStartState *>(state))->loopBackState !=
                     nullptr);
    }

    if (StarLoopEntryState::is(state)) {
      StarLoopEntryState *starLoopEntryState =
          downCast<StarLoopEntryState *>(state);
    ...
    if (LoopEndState::is(state)) {
      checkCondition((downCast<LoopEndState *>(state))->loopBackState !=
                     nullptr);
    }
