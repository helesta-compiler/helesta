...
struct BypassAltsAtnCache final {
  ...
  /// </seealso>
  std::map<std::vector<int32_t>, std::unique_ptr<const atn::ATN>, std::less<>>
      map;
};
...
const atn::ATN &Parser::getATNWithBypassAlts() {
  ...
  return *cache->map
              .insert(std::make_pair(std::vector<int32_t>(serializedAtn.begin(),
                                                          serializedAtn.end()),
                                     std::move(atn)))
              .first->second;
}

tree::pattern::ParseTreePattern
Parser::compileParseTreePattern(const std::string &pattern,
...
tree::pattern::ParseTreePattern
Parser::compileParseTreePattern(const std::string &pattern,
                                int patternRuleIndex, Lexer *lexer) {
...
bool Parser::isExpectedToken(size_t symbol) {
  ...
  while (ctx && ctx->invokingState != ATNState::INVALID_STATE_NUMBER &&
    ...
    const atn::RuleTransition *rt = static_cast<const atn::RuleTransition *>(
...
void Parser::setProfile(bool profile) {
  ...
  if (profile) {
    if (!is<atn::ProfilingATNSimulator *>(interp)) {
      setInterpreter(new atn::ProfilingATNSimulator(
          this)); /* mem-check: replacing existing interpreter which gets
                     deleted. */
    }
